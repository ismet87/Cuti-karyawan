...
class ShiftController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Shift::create([
            ...
            'lama_kerja' => $lama_kerja, // Simpan dalam format angka (contoh: 2.5)
        ]);
    ...
    private function hitungLamaKerja($jam_masuk, $jam_keluar)
    {
        $tgl = date("Y-m-d"); // Gunakan tanggal yang sama
        $masuk = strtotime($tgl . ' ' . $jam_masuk . ':00');
        $keluar = strtotime($tgl . ' ' . $jam_keluar . ':00');
    
        // Menghitung total menit
        $selisih = $keluar - $masuk; // Selisih dalam detik
        $jam = $selisih / 3600; // Menghitung jam dalam desimal (contoh: 1.5 untuk 1 jam 30 menit)
    
        return round($jam, 2); // Bulatkan ke 2 desimal
    }                
